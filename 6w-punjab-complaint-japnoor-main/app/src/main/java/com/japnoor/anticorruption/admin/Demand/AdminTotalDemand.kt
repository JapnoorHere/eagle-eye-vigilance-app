...
import com.japnoor.anticorruption.databinding.FragmentAdminTotalDemandBinding
import java.io.File
...
class AdminTotalDemand : Fragment() ,DemandClick{
    ...
    lateinit var adapter: AdminTotalDemandAdapter
    ...
    override fun onCreateView(
        ...
        var binding = FragmentAdminTotalDemandBinding.inflate(layoutInflater,container,false)
        signUpEntity = SignUpEntity()
        adapter = AdminTotalDemandAdapter(arrayList, this)
    ...
    fun getTotalDemand() {
        ...
        class Get : AsyncTask<Void, Void, Void>() {
            override fun doInBackground(vararg p0: Void?): Void? {
                arrayList.addAll(Datbase.getDatabase(requireContext()).dao().getAllDisplayDemand())
                return null
            }
    ...
    override fun onClick(demandEntity: DisplayDemandEntity) {
        ...
        if(demandEntity.demandEntity?.status=="acc"){
        ...
        else if(demandEntity.demandEntity?.status=="res"){
            dialogBind.fabAccepted.visibility=View.INVISIBLE
            dialogBind.fabRejected.visibility=View.INVISIBLE
        ...
        else if(demandEntity.demandEntity?.status=="rej"){
            dialogBind.fabAccepted.visibility=View.INVISIBLE
            dialogBind.fabRejected.visibility=View.INVISIBLE
            dialogBind.fabResolved.visibility=View.INVISIBLE
            dialogBind.stamp.visibility=View.VISIBLE
        ...
        dialogBind.fabAccepted.setOnClickListener{
            demandEntity.demandEntity?.status="acc"
            changeStatus(demandEntity, dialog)

        }
        dialogBind.fabRejected.setOnClickListener{
            demandEntity.demandEntity?.status="rej"
            changeStatus(demandEntity, dialog)

        }
        dialogBind.fabResolved.setOnClickListener{
            demandEntity.demandEntity?.status="res"
            changeStatus(demandEntity, dialog)
